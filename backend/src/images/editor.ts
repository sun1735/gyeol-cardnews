...
function buildPrompt(
  ...
  if (template === 'product-ad') {
    ...
    lines.push('')
    lines.push('[촬영 스타일 — 이벤트/세일 배너]')
    lines.push('· 강렬한 대비, 컬러 콘트라스트 높게. 어두운 바탕 위에 스포트라이트 느낌도 OK.')
    lines.push('· 중앙에 큰 텍스트(할인율 숫자) 가 올라가므로 가운데는 비교적 비어 있어야 함 — 복잡한 디테일은 네 귀퉁이로.')
    lines.push('· 시선을 끄는 색감(세일감을 주는 레드·네이비·블랙). 너무 파스텔한 톤은 지양.')
    lines.push('· 제품보다 분위기 중심 — 축제·쇼핑·가격표 느낌이 연상되는 장면.')
  }
  if (hasRefs) {
